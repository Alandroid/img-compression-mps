...
def compute_ssim_2d(original: np.ndarray, compressed: np.ndarray) -> float:
    ...
    comp = np.clip(np.asarray(compressed), 0, None)
    data_range = max(orig.max(), comp.max()) - min(orig.min(), comp.min())
    return ssim(orig, comp, data_range=data_range)
...
def calc_mean_std(dict, num_common_points, key_x = "compressionratio_list_disk", key_y = "ssim_list"):
